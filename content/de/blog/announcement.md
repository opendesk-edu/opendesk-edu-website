...
openDesk Edu liefert umfangreiche Dienste in mehreren Kategorien:
...
**Ein-Kommando-Deployment.** Der gesamte Stack wird mit einem einzigen `helmfile apply`-Befehl auf jedem Kubernetes-Cluster bereitgestellt. Kein manuelles Dienst-für-Dienst-Setup, keine fragilen Shell-Skripte. Helmfile orchestriert viele modulare Helm-Charts mit deklarativer Konfiguration.
...
**Modulare Architektur.** Nicht jede Einrichtung benötigt alle Dienste. Das Konfigurationssystem ermöglicht es, nur das zu aktivieren, was Ihre Nutzer benötigen. Betreiben Sie nur ILIAS und Nextcloud, oder setzen Sie den gesamten Stack ein. Die Entscheidung liegt bei Ihnen.
