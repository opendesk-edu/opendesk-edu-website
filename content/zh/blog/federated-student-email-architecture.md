...
- **OpenCloud**（Apache-2.0）：文件存储、共享和协作。
...
开源许可证是混合的（AGPL-3.0、Apache-2.0、MPL-2.0）。AGPL-3.0组件（Stalwart、Matrix）要求通过网络向用户提供的修改必须在相同许可证下发布——这比Apache-2.0更强的copyleft，且增强了而非削弱了主权：修改软件的机构必须共享其修改，防止私有分支侵蚀公共资源。
