...
tags: ["la-suite", "france", "germany", "digital-sovereignty", "open-source", "dinum", "european-collaboration", "public-sector", "higher-education"]
...
In Germany, **openDesk Edu** emerged from a different context — higher education. Hosted by HRZ Marburg and built on the openDesk CE platform, it integrates 25+ open-source services for universities: not just collaboration tools, but learning management systems, scientific computing, and research infrastructure.
...
|-----------|----------|--------------|
| File sync & share | Nextcloud (via Wimi) | Nextcloud (OpenCloud) |
| Document editing | LibreOffice / Collabora | Collabora Online |
| Video conferencing | Jitsi Meet (via Visio) | BigBlueButton + Jitsi |
| Messaging | Tchap (Matrix-based) | Matrix (Element) |
| Email | Calypso (Beta) | Dovecot + Postfix |
| Identity | AgentConnect / ProConnect | Keycloak + DFN-AAI |

The overlap is striking. Both chose Nextcloud for file management, both adopted Matrix-based messaging, both use open-source video conferencing. The European open-source ecosystem is small enough that the same projects keep appearing — and that's a strength, not a weakness.
...
- **openDesk Edu** is operated by HRZ Marburg (University of Marburg's IT center), with support from the Hessian Ministry of Higher Education, and is designed for German universities (~3 million students + staff)
...
**openDesk Edu** is a **federated self-hosting platform**. Each university deploys its own instance on its own Kubernetes cluster. HRZ Marburg runs the reference deployment, but every institution can — and is expected to — operate its own. The GitOps pipeline (ArgoCD + Helmfile) makes this reproducible, but the deployment is yours.

| Aspect | La Suite | openDesk Edu |
|--------|----------|--------------|
| Hosting | Centralized (DINUM) | Federated (per institution) |
| Infrastructure | French sovereign cloud | On-premise Kubernetes |
...
- **Germany** trusts each institution to run its own. The federal structure of German higher education — each university is autonomous — makes a centralized model politically impossible. HRZ Marburg can build a reference, but it cannot mandate adoption.
...
**openDesk Edu** targets **German higher education** — universities, research institutes, student services. The platform includes:
...
- **Visio** — video conferencing (Jitsi-based, GA)
- **Messagerie** — email (Calypso, in beta)
- **Wimi** — collaborative workspace (Nextcloud-based, GA)
- **Tchap** — messaging (Matrix-based, GA)
- **Drive** — file sharing (Nextcloud-based, GA)

The service catalog is intentionally lean — DINUM prioritizes quality and adoption over breadth.

**openDesk Edu** integrates 25+ services and is in production at HRZ Marburg. The platform includes:
...
The scope difference reflects the target: universities need a broader toolset than government offices.
...
**openDesk Edu** is a **community-driven project**. While HRZ Marburg leads development, the project is open on GitHub and Codeberg, accepts contributions, and publishes its roadmap publicly. The contributor agreement, the community-of-practice meetings, and the transparent gap analysis (the ZKI compliance work) all reflect a different governance model — one where institutions collaborate rather than receive a service.
...
1. **Centralized evaluation lowers the barrier.** La Suite's single instance means a French ministry can try the platform without deploying anything. openDesk Edu's self-hosting model requires Kubernetes expertise — a high barrier for smaller institutions. A shared evaluation instance (as proposed in the DFN-AAI article) would address this.

2. **Lean service catalog.** La Suite focuses on 5 core services and does them well. openDesk Edu's 25+ services are a strength but also a maintenance burden. Not every university needs all of them — a tiered deployment model (core, extended, research) could help.

3. **Government mandate as an adoption driver.** La Suite benefits from an explicit French government mandate for sovereign digital tools. openDesk Edu relies on individual university adoption — slower, but more sustainable.
...
2. **Federated self-hosting for research data.** Research data often cannot leave the institution (ethical, legal, or technical constraints). La Suite's centralized model makes this harder. openDesk Edu's per-institution deployment gives each university full control over sensitive research data.
...
- La Suite and openDesk Edu share the same open-source components (Nextcloud, Collabora, Matrix, Jitsi/BigBlueButton)
...
- **openDesk Edu** runs on on-premise Kubernetes at HRZ Marburg, which could federate with GAIA-X infrastructure
...
1. **A joint workshop** on sovereign digital workplaces in European public administration, co-hosted by DINUM and HRZ Marburg
2. **A shared component matrix** — mapping which open-source services each platform uses, identifying opportunities for joint development
3. **eduGAIN integration for La Suite** — extending AgentConnect/ProConnect to participate in the global research federation
4. **A cross-border evaluation** — letting a French university pilot openDesk Edu and a German agency pilot La Suite, to learn from both models
5. **A joint compliance mapping** — mapping ANSSI guidance to BSI IT-Grundschutz, creating a European security baseline for sovereign digital workplaces
...
France chose centralization; Germany chose federation. France chose a lean service catalog; Germany chose breadth. France chose a government mandate; Germany chose community adoption. Both choices are legitimate — and both have something to teach the other.
...
*openDesk Edu is an open-source project. We welcome contributions from across Europe — not just Germany. If you're working on sovereign digital infrastructure in France, Belgium, the Netherlands, or anywhere else, we'd love to hear from you.*
